...
#endif
	bool was_class_name = JS::GetReservedSlot(obj, 1).toBoolean();

	if (was_class_name) {
		struct el_dom_html_collection *ns = JS::GetMaybePtrFromReservedSlot<el_dom_html_collection>(obj, 0);
...
JSClass htmlCollection_class = {
	"htmlCollection",
	JSCLASS_HAS_RESERVED_SLOTS(2) | JSCLASS_HAS_CACHED_PROTO(JSProto_Array),
	&htmlCollection_ops
};
...
getCollection_common(JSContext *ctx, void *node, bool was_class_name)
{
	...
	JS::SetReservedSlot(el, 1, JS::BooleanValue(was_class_name));
