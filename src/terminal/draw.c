...
#define clear_screen_char_color(schar) \
	do { memset((schar)->c.color, 0, SCREEN_COLOR_SIZE); schar->is_default_fg_color = 0; schar->is_default_bg_color = 0; } while (0)
#else
#define clear_screen_char_color(schar) \
	do { (schar)->c.color[0] = 0; schar->is_default_fg_color = 0; schar->is_default_bg_color = 0; } while (0)
#endif
