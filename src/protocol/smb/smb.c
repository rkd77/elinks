...
static ssize_t
smb_read_data(struct connection *conn, int sock, char *dst)
{
	ssize_t r;
...
smb_read_text(struct connection *conn, int sock)
{
	...
	if (!conn->from) set_connection_state(conn, connection_state(S_GETH));
	si->textlen += r;
}
...
static char *
find_strs(char *line, char *str1,
	  char *str2)
{
	char *p = strstr(line, str1);
...
#define MAX_SMBCLIENT_ARGS 32
		...
		if (*share) {
			...
			if (!dirlen || dir[dirlen - 1] == '/' || dir[dirlen - 1] == '\\') {
				...
				char *s = straconcat("get \"", dir, "\" -", NULL);
				char *ss = s;
		...
		optstr = get_opt_str("protocol.smb.credentials", NULL);
		if (optstr[0]) {
			v[n++] = "-A";
			v[n++] = optstr;
		}
